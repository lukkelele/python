...
# FIXME
def k_means(points, k):     # pyright: ignore
    """
    ...
    for cluster in clusters:
        c = clusters.index(cluster)
        for p in cluster:
            plt.scatter(p[0], p[1], c=c, edgecolors='k', cmap=cmap)
...
def k_means(points, k, n=5):
    ...
    for i in range(1, k):
        ...
        plt.scatter(centroids[i][0], centroids[i][1], s=300, edgecolors='k', alpha=0.89)
